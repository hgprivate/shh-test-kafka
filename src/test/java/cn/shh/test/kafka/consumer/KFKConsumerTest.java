 ...
 * 消息消费者
 * <p>
 * 概述：Kafka是通过拉取消息来消费。KafkaConsumer线程非安全。
 * </p>
 * <p>
 * 常用方法：
 *   - pause(): 控制消费；
...
public class KFKConsumerTest {
     ...
     *   - 集合方式：subscribe(Collection)，例如：consumer.subscribe(Arrays.asList("first"));
     *   - 正则表达式方式：subscribe(Pattern)，例如：consumer.subscribe(Pattern.compile("first*"));
     * </p>
     * 要想订阅主题中的特定分区可用assign()方法实现。取消订阅某个主题可以使用consumer.unsubscribe()方法。
     * 如果subscribe方法和assign方法入参集合为空，那么其作用相当于调用unsubscribe()方法。
     * <p>
     * 集合订阅方式、正则订阅方式、指定分区订阅方式 分别代表三种订阅状态：
     *   - AUTO_TOPICS
     *   - AUTO_PATTERN
     *   - USER_ASSIGNED
     * 如果没有订阅，那么订阅状态为 NONE。该三种状态互斥，一个消费者只能使用其中的一种方式，否则会抛异常。
     * </p>
     * <p>
     * 消费者自动再均衡：subscribe()方法具备 消费者自动再均衡功能，而assign()方法不具备该功能。
     * </p>
     * <p>
     * 注意事项：
     ...
     * 提交消费位移
     *   - 自动提交相关参数：
     *     - enable.auto.commit 表示是否开启自动提交offset。默认开启。
     *     - auto.commit.interval.ms 表示自动提交的时间间隔。默认5秒。
     *   - 手动提交分两种：
     *     - 同步提交：commitSync()方法
     *     - 异步提交：commitAsync()方法
     * </p>
